...
public class XML2EQ {
		...
		try{
			for(File f: xmlfiles){
				...
				while(it.hasNext()){
					...
					List<Element> relations = XPath.selectNodes(root,".//relation");
	...
	private String getStructureName(Element root, String structid) {
		try{
			...
			return structure.getAttributeValue("constraint")+" "+structure.getAttributeValue("name").trim();
		}catch(Exception e){
			e.printStackTrace();
		}
		return null;
	...
	private void outputEQa4Structure(String src, Element root, String text, Element struct) {
		String structname = struct.getAttributeValue("constraint")+" "+struct.getAttributeValue("name").trim();
		try{
	...
	public static void main(String[] args) {
		...
		String database = "";
		String outputtable = "";
		XML2EQ x2e = new XML2EQ(src, database, outputtable);
		x2e.outputEQs();
	}
